...
class Client(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.author != self.user:
            ...
            if message.content == 'BDAY' and global bdayPerson == 'nobody':
                response = ('it aint nobodys bday bitch')
                await message.channel.send(response)
            elif message.content == 'BDAY' and global bdayPerson != 'nobody':
                response = ('HAPPY BDAY TO ' + global bdayPerson + '!!!!!!!!  buy them shit')
                await message.channel.send(response)
            if message.content == 'NEXT BDAY':
                response = ('The next goose bday is ' + global nextPerson + '\'s on ' + global nextBday + '!!!!!!!!!!!!!!!!!!  Get hype bout it')
                await message.channel.send(response)
